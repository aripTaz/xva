...
VALIDITY () {
    ...
    Exp1=$(curl https://raw.githubusercontent.com/rizyulstore/ip/main/ip | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl https://raw.githubusercontent.com/rizyulstore/ip/main/ip | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://github.com/rizyulstore/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://github.com/rizyulstore/ip | grep $MYIP | awk '{print $4}')
...
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(curl https://github.com/rizyulstore/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
TIME="10"
NAMES=$(curl -sS https://raw.githubusercontent.com/rizyulstore/main/ip| grep $MYIP | awk '{print $2}')
EXPSC=$(curl -sS https://raw.githubusercontent.com/rizyulstore/ip/main/ip | grep $MYIP | awk '{print $4}')
CHATID="1906388998"
LOCAL_DATE="/usr/bin/"
